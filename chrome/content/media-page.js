...
window.mediaPage = {
  ...
  */
  playAlbum : function MF_playAlbum(){
    ...
    if (typeof(gPPS)!='undefined' && gPPS && gPPS.playingView){
      try{
        playingItem = gPPS.playingView.mediaList.getItemByGuid(gPPS.currentGUID);
        playingAlbum = new Album(playingItem);
        playingTrack = playingItem.getProperty(SBProperties.trackName);
      }catch(e){}
    } else if (typeof(gMM) !="undefined" && gMM && gMM.sequencer.view){
  ...
  _getAlbumArtFromMediaItem: function MF_getAlbumArtFromMediaItem(aMediaItem){
    ...
    if (mAlbumArtUrl == null || mAlbumArtUrl == "") {
      if (this._albumArtService){
        ...
        mAlbumArtUrl = "chrome://songbird/skin/album-art/default-cover.png";
      }
    }
    return mAlbumArtUrl;
  },
  ...
  if (typeof arg1 == 'string'){
    ...
    if (typeof artist == 'string')
      if (typeof albumartist == 'string' && albumartist!="")
        ...
        this.artist = artist;
  } else {//a media item was passed in
...
Album.prototype.equals = function A_equals(aAlbum){
  if (aAlbum!=null){
    if (Album.prototype.ignoreAlbumArtist)
      ...
      return aAlbum.name == this.name && aAlbum.artist == this.artist;
  }
  return false;
}
